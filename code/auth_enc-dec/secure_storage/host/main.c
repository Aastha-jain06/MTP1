...
/* Read PIN from user (with hidden input) */
static int read_pin(char *pin, size_t max_len)
{
	...
	if (tcgetattr(STDIN_FILENO, &old_term) != 0) {
		printf("Warning: Cannot hide PIN input\n");
		printf("Enter PIN: ");
		if (fgets(pin, max_len, stdin) == NULL)
			return -1;
	} else {
		/* Disable echo */
		new_term = old_term;
		...
		ret = (fgets(pin, max_len, stdin) != NULL) ? 0 : -1;
...
/* Set PIN in TEE */
static TEEC_Result set_pin_tee(struct test_ctx *ctx, const char *pin)
{
	TEEC_Operation op;
	uint32_t origin;
	TEEC_Result res;
	...
	op.params[0].tmpref.buffer = (void *)pin;
	op.params[0].tmpref.size = strlen(pin);
	
	res = TEEC_InvokeCommand(&ctx->sess,
				 TA_SECURE_STORAGE_CMD_SET_PIN,
				 &op, &origin);
...
/* Verify PIN with TEE */
static TEEC_Result verify_pin_tee(struct test_ctx *ctx, const char *pin,
                                   uint32_t *success, uint32_t *attempts_left)
{
	TEEC_Operation op;
	uint32_t origin;
	TEEC_Result res;
	...
	res = TEEC_InvokeCommand(&ctx->sess,
				 TA_SECURE_STORAGE_CMD_VERIFY_PIN,
				 &op, &origin);
	
	if (res == TEEC_SUCCESS || res == TEEC_ERROR_ACCESS_DENIED) {
		*success = op.params[1].value.a;
		*attempts_left = op.params[2].value.a;
	}
	
	return res;
}

/* Prompt user to set up PIN */
static int setup_pin(struct test_ctx *ctx)
{
	char pin1[PIN_MAX_LENGTH + 2];
	char pin2[PIN_MAX_LENGTH + 2];
	size_t len;
	TEEC_Result res;
	
	printf("\n=== PIN SETUP ===\n");
	printf("Please set a PIN (%d-%d digits)\n", PIN_MIN_LENGTH, PIN_MAX_LENGTH);
	
	while (1) {
		/* Get PIN */
		if (read_pin(pin1, sizeof(pin1)) != 0) {
		...
		if (len < PIN_MIN_LENGTH || len > PIN_MAX_LENGTH) {
			printf("❌ PIN must be %d-%d digits. Try again.\n",
			       PIN_MIN_LENGTH, PIN_MAX_LENGTH);
			continue;
		}
		...
		if (!valid) {
			printf("❌ PIN must contain only digits. Try again.\n");
			continue;
		}
		
		/* Confirm PIN */
		printf("Confirm PIN: ");
		if (read_pin(pin2, sizeof(pin2)) != 0) {
			printf("Error reading PIN\n");
			return -1;
		}
		...
		/* Set PIN in TEE */
		res = set_pin_tee(ctx, pin1);
		if (res != TEEC_SUCCESS) {
			printf("❌ Failed to set PIN: 0x%x\n", res);
			return -1;
		}
...
/* Prompt user to verify PIN */
static int verify_pin_prompt(struct test_ctx *ctx)
{
	char pin[PIN_MAX_LENGTH + 2];
	uint32_t success, attempts_left;
	TEEC_Result res;
	int max_tries = PIN_MAX_ATTEMPTS;
	...
	for (int try = 0; try < max_tries; try++) {
		...
		res = verify_pin_tee(ctx, pin, &success, &attempts_left);
		
		/* Clear PIN from memory */
		memset(pin, 0, sizeof(pin));
		
		if (res == TEEC_ERROR_ACCESS_DENIED && success == 0) {
			if (attempts_left == 0) {
			...
			printf("❌ Incorrect PIN. %u attempt(s) remaining.\n",
			       attempts_left);
			continue;
		} else if (res == TEEC_ERROR_BAD_STATE) {
			printf("❌ PIN not set up\n");
			return -1;
		} else if (res != TEEC_SUCCESS) {
			printf("❌ PIN verification failed: 0x%x\n", res);
			return -1;
		}
...
int main(int argc, char *argv[])
{
	...
	printf("=======================================================\n");
	printf("  OP-TEE File Encryption/Decryption with PIN Auth\n");
	...
	/* Verify PIN before encryption */
	if (verify_pin_prompt(&ctx) != 0) {
		printf("✗ Authentication failed\n");
		terminate_tee_session(&ctx);
		return 1;
	}
	...
	/* Verify PIN again before decryption */
	if (verify_pin_prompt(&ctx) != 0) {
		printf("✗ Authentication failed\n");
		goto cleanup;
	}
